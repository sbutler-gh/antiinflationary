<script>
    ...
    //   import WelcomePopup from "$lib/WelcomePopup.svelte";
      ...
      let chart_labels = [...Array(Math.ceil(payback_years)+4).keys()];
      ...
      let savings_shared_data = {
        ...
        datasets: [
          {
            values: [...Array((Math.ceil(payback_years)+4)).keys()].map(multiplySavingsShared),
            chartType: 'line',
            lineOptions: { regionFill: 1}
          },
          {
      ...
      <span class="range-div">${dollar_savings_kept} <input type="range" bind:value={dollar_savings_kept} min={0} max={max_dollar_savings} on:input={function() { savings_kept_data.datasets[0].values = [...Array((Math.ceil(payback_years)+4)).keys()].map(multiplySavingsKept); savings_shared_data.datasets[0].values = [...Array((Math.ceil(payback_years)+4)).keys()].map(multiplySavingsShared)}}></span>
      
      <span style="font-weight: 600">{per_time_unit_label}</span> and pay back the {infrastructure} in <span class="range-div">{payback_years}<input type="range" bind:value={payback_years} min={0} max={100} on:mouseup={function() {savings_kept_data.labels = [...Array(Math.ceil(payback_years)+4).keys()]; savings_shared_data.labels = savings_kept_data.labels; savings_kept_data.datasets[0].values = [...Array((Math.ceil(payback_years)+4)).keys()].map(multiplySavingsKept); savings_shared_data.datasets[0].values = [...Array((Math.ceil(payback_years)+4)).keys()].map(multiplySavingsShared)}}></span> years — giving the project a
      <!-- <span class="range-div">{new_arr.toFixed(2)}%<input type="range" bind:value={new_arr} min={0} max={100} on:change={() => {payback_years = ((new_arr * (1/new_roi)) / 100)}}></span> -->
  <strong>{new_arr.toFixed(2)}% annual rate of return. {#if new_arr.toFixed(2) > 0}
        <svg xmlns="http://www.w3.org/2000/svg" class="return-check icon icon-tabler icon-tabler-circle-check" width="32" height="32" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ffffff" fill="#56e156" style="vertical-align: middle; display: inline;" stroke-linecap="round" stroke-linejoin="round">
      ...
      <h2 style="text-align: center" class="">Interested in joining a project like this?</h2>
      </div>

      <!-- <div class="bottom"> -->
      <!-- <h2 class="">Interested in joining a project like this?</h2> -->
      <!-- </div> -->
